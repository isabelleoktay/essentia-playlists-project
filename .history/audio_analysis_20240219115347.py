...
def get_music_styles(discogs_embeddings):
    model = es.TensorflowPredict2D(graphFilename="models/genre_discogs400-discogs-effnet-1.pb", input="serving_default_model_Placeholder", output="PartitionedCall:0")
    predictions = model(discogs_embeddings)

    return predictions


def classify_voice_or_instrument(discogs_embeddings, musiCNN_embeddings):
    ...
    print(discogs_predictions)

    musiCNN_model = es.TensorflowPredict2D(graphFilename="models/voice_instrumental-msd-musicnn-1.pb", output="model/Softmax")
    musiCNN_predictions = musiCNN_model(musiCNN_embeddings)
    print(musiCNN_predictions)
