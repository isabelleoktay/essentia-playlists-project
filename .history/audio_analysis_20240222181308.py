...
def get_tempo(mono_audio):
    try: 
        bpm, _, _, _, _ = RhythmExtractor2013()(mono_audio)
        return bpm
...
def get_key(mono_audio):
    ...
    try:
        for profile in keys_and_scales:
            key, scale, _ = KeyExtractor(profileType=profile)(mono_audio)
...
def get_embeddings(mono_audio):
    ...
    try:
        musiCNN_model = TensorflowPredictMusiCNN(graphFilename="models/msd-musicnn-1.pb", output="model/dense/BiasAdd")
...
def get_music_styles(discogs_embeddings):
    try:
        discogs_model = TensorflowPredict2D(graphFilename="models/genre_discogs400-discogs-effnet-1.pb", input="serving_default_model_Placeholder", output="PartitionedCall:0")
...
def classify_voice_or_instrument(discogs_embeddings):
    try:
        discogs_model = TensorflowPredict2D(graphFilename="models/voice_instrumental-discogs-effnet-1.pb", output="model/Softmax")
        discogs_predictions = discogs_model(discogs_embeddings)
        discogs_mean_predictions = np.mean(discogs_predictions, axis=0)
...
def get_danceability(discogs_embeddings):
    try:
        discogs_model = TensorflowPredict2D(graphFilename="models/danceability-discogs-effnet-1.pb", output="model/Softmax")
        discogs_predictions = discogs_model(discogs_embeddings)
        discogs_mean_predictions = np.mean(discogs_predictions, axis=0)
...
def get_arousal_and_valence(musiCNN_embeddings):
    try:
        musiCNN_model = TensorflowPredict2D(graphFilename="models/emomusic-msd-musicnn-2.pb", output="model/Identity")
...
def load_audio(filename):
    ...
    try:
        stereo_audio, _, num_channels, _, _, _ = AudioLoader(filename=filename)()
    except Exception as e:
